...
Per spec: avoid excessive hyperparameter search that makes this
unnecessarily slow — grid kept reasonably scoped.
"""
import pandas as pd
import time
...
if __name__ == "__main__":
    ...
    pipe = Pipeline([
        ...
        ("clf", XGBClassifier(scale_pos_weight=scale_pos_weight, random_state=RANDOM_SEED, eval_metric="logloss")),
    ])

    # Scoped grid — kept reasonable per spec, not excessive
    param_grid = {
    ...
    joblib.dump(best_pipe, f"{RESULTS_DIR}/xgboost_model.pkl")

    with open(f"{RESULTS_DIR}/xgboost_results.json", "w") as f:
        json.dump(result, f, indent=2)

    print(f"\nSaved to {RESULTS_DIR}/xgboost_results.json")